...
`get_companies` with `ids` in pages of `harmonic.page_size_enrich` (6) and `field_groups` = `["name_id_description_headcount_website", "location", "founders_ceo", "external_profiles", "date_added_to_harmonic", "notes_and_list_membership", "contact"]` (`contact` adds company emails, phone and each founder's primary email). For companies without a Harmonic id, pass `identifiers: [{"website_domain": …}]`. If a founder record lacks education or experience, call `get_people` with their `linkedin_urls` and `field_groups` = `["basic", "experience", "education", "location"]`. When `to_enrich` exceeds 40, split across subagents in chunks of 30 using `references/agent-brief.md` (each writes `state/candidates/<CODE>/enriched-<chunk>.json`), then `python3 scripts/merge_enriched.py`; never drop companies silently. Anything left unenriched is listed in the run log and picked up next run.
...
### 7. Report
Build `state/candidates/report-data-<date>.json` (main list = Confirmed plus well-supported Likely; thin proxies to the appendix; add `probe_note` and `run_log` strings) and `state/candidates/known-status-<date>.json` (status, detail, warm path per company), then run `python3 scripts/render_report.py --data … --known … --date <date> [--contacts …]`. The layout is fixed:
1. **Latest batch spotlight**: every qualifying company from the newest tracked batch, new and in-motion alike.
2. **New to Lightspeed, all batches, all tiers in one table**: company, batch, what it does, why included, region, tier, founders as LinkedIn links with emails when Harmonic has them, contacts (website, YC page, company email, Harmonic), warm path.
